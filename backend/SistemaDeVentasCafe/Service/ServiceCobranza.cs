    ...
    public class ServiceCobranza : IServiceGeneric<CobranzaUpdateDto, CobranzaCreateDto>
    {
        ...
        public async Task<APIResponse> Eliminar(int id)
        {
            try
            {
                ...
                //Nuevo metodo para sacarle el estado de pago a la factura asociada
                ...
                {
                    {
                        _logger.LogError("No existe una factura con ese numero.");
                        return Utilidades.NotFoundResponse(_apiresponse);
                    }
                }
                factura.EstadoPago = false;

                await _unitOfWork.repositoryFactura.Actualizar(factura);// Actualizo el estado de pago
